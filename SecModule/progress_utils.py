...
class ProgressManager:
    ...
    def start_spinner(self,desc_start='Caricamento',desc_end='Fatto!', charset='braille'):
        spinner_sets = {
            'classic': ['|', '/', '-', '\\'],
            'braille': ['⠋','⠙','⠹','⠸','⠼','⠴','⠦','⠧','⠇','⠏'],
            'dots': ['⠁','⠂','⠄','⡀','⢀','⠠','⠐','⠈']
        ...
        def _run():
            for c in itertools.cycle(chars):
                if self._stop_flag:
                    break
                sys.stdout.write(f'\r{desc_start} {c}')
                sys.stdout.flush()
                time.sleep(0.1)
            sys.stdout.write(f'\r{desc_end}\n')
...
class Spinner:
    ...
    def update(self, description: str):
        """Aggiorna lo spinner con una nuova descrizione sulla stessa riga"""
        sys.stdout.write(f"\r{description}{next(self.spinner)}")
        sys.stdout.flush()
